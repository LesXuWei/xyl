...
/*			if (!hasTouch) {
	...
	SwipeView.prototype = {
		...
		onMoveEnd: function(fn) {
			this.wrapper.addEventListener('swipeview-onMoveEnd', fn, false);
			this.customEvents.push(['onMoveEnd', fn]);
		},
...
/*			if (!hasTouch) {
		...
		goToPage: function (p) {
			...
			this.masterPages[this.currentMasterPage].style.visibility = "visible";
			$(this.masterPages[this.currentMasterPage]).append(this.__getShade(p));
			this.__flip();
		},
		...
		*
		*/
		__getShade : function(index) {
			if (this.getShade) {
				var re = this.getShade(index);
			...
			return "<div class='swipeview_shade'><div></div></div>";
		},
		
		__hasShade : function(mpIndex) {
			return $(this.masterPages[mpIndex]).find(".swipeview_shade").length > 0;
		},
...
/*			var matrix = getComputedStyle(this.slider, null).webkitTransform.replace(/[^0-9-.,]/g, '').split(',');
		...
		__move: function (e) {
			...
			this.directionX = (this.pointX - this.startX) > 0 ? 1 : (this.pointX - this.startX) < 0 ? -1 : 0;
...
/*			if (newX > 0 || newX < this.maxX) {
		...
		__end: function (e) {
			...
			if (dist < this.snapThreshold) {
				...
				if(this.__hasShade(this.currentMasterPage)) {
					this.__flip();
				}
				return;
			}
		...
		__checkPosition: function () {
			...
			this.pageIndex = parseFloat(this.pageIndex);
			this.page = parseFloat(this.page);
			// Flip the page
			...
			if (!this.options.loop) {
				if (this.pageIndex == this.options.numberOfPages - 1 || this.pageIndex == 0) {
					this.masterPages[pageFlip].style.visibility = "hidden";
				}
				else {
					this.masterPages[pageFlip].style.visibility = "";
					if (this.options.numberOfPages > 3) {
						$(this.masterPages[pageFlip]).append(this.__getShade(this.directionX > 0 ? this.pageIndex - 1 : this.pageIndex + 1));
					}
				}
				// if (newX === 0 || newX == this.maxX) {
				// }
